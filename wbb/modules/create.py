...
from wbb import SUDOERS, USERBOT_PREFIX, BOT_USERNAME, app2
...
async def create(_, message):
    if len(message.command) >= 3:
        ...
        if group_type == "b":  # for basicgroup
            ...
            await edit_or_reply(message, text=f"**Basicgroup Created: [{group_name}]({link['invite_link']})**")
        elif group_type == "s":  # for supergroup
            _id = await app2.create_supergroup(group_name, desc)
            link = await app2.get_chat(_id['id'])
            await edit_or_reply(message, text=f"**Supergroup Created: [{group_name}]({link['invite_link']})**")
        elif group_type == "c":  # for channel
            _id = await app2.create_channel(group_name, desc)
            link = await app2.get_chat(_id['id'])
            await edit_or_reply(message, text=f"**Channel Created: [{group_name}]({link['invite_link']})**")
    else:
        await edit_or_reply(message, text="__**.create (b|s|c) Name**__")
    sleep(10)
